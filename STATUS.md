...
## Part 1 · Open defects — archived

Three pages of open defects lived here, and by 2026-08-29 every one of them was
about a stage that no longer exists: `_own_input`, `evidence_discriminates`,
`_second_chance`, `bind`, `split`, the critic and its repair loop. As a work
list it was fiction, and a reader taking it at its word would have gone looking
for files that had been deleted.

Moved verbatim to
[docs/archive/STATUS_PART1_OPEN_DEFECTS.md](docs/archive/STATUS_PART1_OPEN_DEFECTS.md).
The reasoning is worth keeping -- it is the record of how the real cause was
found, and several of those defects were fixed correctly while still fighting
downstream of a recorder that never captured the page.

**What is actually open is [docs/COMPLAINT.md](docs/COMPLAINT.md)**, written
after the rebuild against real sessions.
...
investigation, and `ROUTINE_TOOLS` — since deleted with the step library —
filtered by tool NAME, which cannot catch it —
