...
#: Read-only, and four rather than the author's six. `get_network` and
#: `get_narration` are absent because neither answers any of the five checks:
#: what the tester said is already quoted in the prompt, and a request is not
#: what a QA lead reads a scenario against.
JUDGE_TOOLS = ["get_diff", "get_snapshot", "see", "find_text"]
...
#: The five, and the ids are the vocabulary `evals/RUBRIC.md` already uses, so a
#: finding here and a finding there are the same kind of thing.
CHECKS = (
        ...
        for step in case.steps:
            ...
            for assertion in step.assertions:
                ...
                lines.append(
                    ...
                    f"({evidence.kind} at {evidence.eventId})"
                )
            if step.whyNot:
